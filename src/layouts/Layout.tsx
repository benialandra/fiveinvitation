    ...
    <div className="min-h-screen bg-white transition-colors duration-300 dark:bg-[#0A0A0B] text-gray-900 dark:text-[#E5E5E5] flex flex-col font-sans relative overflow-x-hidden">
      ...
      {csOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
           <div className="bg-white dark:bg-[#111] border border-black/10 dark:border-white/10 w-full max-w-lg rounded-[32px] p-8 relative shadow-2xl animate-in zoom-in-95 duration-300">
              ...
              <div className="space-y-6 mb-8">
                <div>
                   <h4 className="text-[10px] uppercase tracking-widest text-[#C5A059] font-bold mb-4">{lang === 'id' ? 'Alur Pemesanan' : 'Order Workflow'}</h4>
                   <div className="space-y-4">
                      <div className="flex gap-4">
                         <div className="flex flex-col items-center">
                            <span className="w-6 h-6 rounded-full bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 flex items-center justify-center text-xs text-gray-900 dark:text-white font-medium">1</span>
                         </div>
                         <div>
                            <p className="text-sm font-semibold text-gray-900 dark:text-white">{lang === 'id' ? 'Pilih Tema & Isi Data' : 'Select Theme & Enter Data'}</p>
                            <p className="text-xs text-gray-500 dark:text-white/50">{lang === 'id' ? 'Pilih desain dan isi form detail mempelai.' : 'Choose design and fill wedding details.'}</p>
                         </div>
                      </div>
                      <div className="flex gap-4">
                         <div className="flex flex-col items-center">
                      ...
                      <div className="flex gap-4">
                         <div className="flex flex-col items-center">
                            <span className="w-6 h-6 rounded-full bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 flex items-center justify-center text-xs text-gray-900 dark:text-white font-medium">3</span>
                         </div>
                         <div>
                            <p className="text-sm font-semibold text-gray-900 dark:text-white">{lang === 'id' ? 'Undangan Terbit' : 'Invitation Published'}</p>
                            <p className="text-xs text-gray-500 dark:text-white/50">{lang === 'id' ? 'Link undangan Anda langsung aktif dan siap disebar.' : 'Your link is instantly active and ready to share.'}</p>
                         </div>
                      </div>
                   </div>
                </div>
              ...
              <div className="space-y-3">
                 <a href="https://wa.me/6281234567890" target="_blank" rel="noopener noreferrer" className="flex items-center justify-center w-full h-12 rounded-xl bg-green-500 text-white font-medium text-sm hover:bg-green-600 transition-colors">
                    WhatsApp Support
                 </a>
      ...
      <div className="fixed bottom-6 right-6 flex flex-col gap-4 z-50">
        {showScrollTop && (
          <button 
             ...
             className="w-14 h-14 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-full shadow-xl flex items-center justify-center hover:scale-105 transition-transform animate-in fade-in slide-in-from-bottom-4 duration-300"
          >
             <ArrowUp size={24} />
          </button>
        )}
        <button 
           ...
           className="w-14 h-14 bg-[#C5A059] text-white rounded-full shadow-2xl shadow-[#C5A059]/30 flex items-center justify-center hover:scale-105 transition-transform relative overflow-hidden"
        >
