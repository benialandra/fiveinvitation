...
export default function Dark07({ data, guestName, lang = 'id' }: any) {
  ...
  const coverImage = data?.cover_image || "https://images.unsplash.com/photo-1493225457124-a1a2a5f5f9af?q=60&w=600";
  const heroImage = data?.hero_image || "https://images.unsplash.com/photo-1493225457124-a1a2a5f5f9af?q=60&w=600";
  
  const weddingDate = new Date(dateStr);
  const currentLocale = lang === 'en' ? localeEn : localeId;

  // Configuration for Dark theme
  const colors = { primary: '#8B5CF6', background: '#09090B', text: '#FAFAFA' };
  const fonts = { heading: "'Space Grotesk', sans-serif", body: "'Inter', sans-serif" };
  ...
  return (
    <div className="bg-zinc-950 text-zinc-50 py-19 px-5" style={{ fontFamily: fonts.body, backgroundColor: colors.background, color: colors.text }}>
        ...
        {!isOpen && (
          ...
          >
            ...
            <div className="relative z-10 max-w-xl mx-auto flex flex-col items-center justify-center h-full pt-10">
              ...
              <motion.button
                onClick={handleOpen}
                className="hover:scale-105 hover:ring-4 hover:ring-purple-500/30 px-8 py-4 rounded-full font-medium tracking-widest text-xs uppercase shadow-xl transition-all duration-500 flex items-center gap-3 border"
