...
export default function Islamic01({ data, guestName, lang = 'id' }: any) {
  ...
  const heroImage = data?.hero_image || "https://images.unsplash.com/photo-1519741497674-611481863552?q=60&w=600";
  ...
  const fonts = { heading: "Amiri", body: "Noto Kufi Arabic" };
  ...
  return (
    <div className="relative overflow-hidden selection:bg-[#D4AF37]/30 py-12 px-4" style={{ fontFamily: fonts.body, backgroundColor: colors.background, color: colors.text }}>
      <IslamicParticles />
      ...
      {isOpen && (
        <SmoothScrollLayout>
          ...
          <div className="relative z-10 opacity-95 scale-100"><SharedCountdown targetDate={weddingDate} colors={colors} fonts={fonts} /></div>

          <div className="relative z-10 opacity-95 scale-100"><SharedStory storyText={storyText} colors={colors} fonts={fonts} /></div>

          <div className="overflow-hidden bg-transparent"><SharedGallery colors={colors} fonts={fonts} images={galleryImages} /></div>

          <div className="relative z-10 opacity-95 scale-100"><SharedRSVP colors={colors} fonts={fonts} /></div>

          <div className="overflow-hidden bg-transparent"><SharedGift colors={colors} fonts={fonts} bankName={bankName} bankAccount={bankAccount} bankOwner={bankOwner} /></div>
