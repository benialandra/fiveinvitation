...
export default function Islamic05({ data, guestName, lang = 'id' }: any) {
  ...
  const coverImage = data?.cover_image || "https://images.unsplash.com/photo-1584551246679-0daf3d275d0f?q=60&w=600";
  const heroImage = data?.hero_image || "https://images.unsplash.com/photo-1584551246679-0daf3d275d0f?q=60&w=600";
  
  const weddingDate = new Date(dateStr);
  const currentLocale = lang === 'en' ? localeEn : localeId;

  // Configuration for Islamic theme
  const colors = { primary: '#166534', background: '#F0FDF4', text: '#14532D' };
  const fonts = { heading: "'Amiri', serif", body: "'Noto Serif Arabic', serif" };
  ...
  return (
    <div className="relative w-full py-10 px-6" style={{ fontFamily: fonts.body, backgroundColor: colors.background, color: colors.text }}>
        ...
        {!isOpen && (
          ...
          >
            ...
            <div className="relative z-10 max-w-xl mx-auto flex flex-col items-center justify-center h-full pt-10">
              ...
              <motion.button
                onClick={handleOpen}
                className="hover:shadow-xl hover:-translate-y-0.5 px-8 py-4 rounded-full font-medium tracking-widest text-xs uppercase shadow-xl transition-all duration-500 flex items-center gap-3 border"
