...
const CountdownTimer = React.memo(({ targetDate, colors, fonts }: { targetDate: string, colors: any, fonts: any }) => {
...
export default function MasterTheme({
  ...
  let config: any = defaultJSONConfig;
  try {
    config = typeof config_json === 'string' && config_json.trim() ? JSON.parse(config_json) : (config_json || defaultJSONConfig);
  } catch (e) {
    console.warn("Invalid config_json passed to MasterTheme", e);
  }
              ...
              <div className="flex flex-col md:flex-row justify-center items-center gap-12 md:gap-24">
                <div className="flex flex-col items-center">
                  <div className="w-40 h-40 rounded-full overflow-hidden mb-6 border-4 shadow-xl" style={{ borderColor: colors?.primary }}>
                    <img loading="lazy" src={finalGroomImage} alt="Groom" className="w-full h-full object-cover" />
                  </div>
                ...
                <div className="flex flex-col items-center">
                  <div className="w-40 h-40 rounded-full overflow-hidden mb-6 border-4 shadow-xl" style={{ borderColor: colors?.primary }}>
                    <img loading="lazy" src={finalBrideImage} alt="Bride" className="w-full h-full object-cover" />
                  </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full perspective-1000">
                {galleryImages.map((img: string, i: number) => (
                  <div key={i} className="aspect-[3/4] w-full rounded-2xl overflow-hidden shadow-lg relative group transition-all duration-500 ease-out hover:-translate-y-2 hover:shadow-2xl bg-white border" style={{ borderColor: colors?.primary + '20' }}>
                    <img loading="lazy" src={img} alt={`Gallery ${i}`} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                  </div>
                ))}
              </div>
