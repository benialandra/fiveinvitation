...
export default function Tropical01({ data, guestName, lang = 'id' }: any) {
  ...
  const coverImage = data?.cover_image || "https://images.unsplash.com/photo-1530103862676-de8c9debad1d?q=60&w=600";
  const heroImage = data?.hero_image || "https://images.unsplash.com/photo-1530103862676-de8c9debad1d?q=60&w=600";
  
  const weddingDate = new Date(dateStr);
  const currentLocale = lang === 'en' ? localeEn : localeId;

  // Configuration for Tropical theme
  const colors = { primary: '#0D9488', background: '#F0FDFA', text: '#115E59' };
  const fonts = { heading: "'Montserrat', sans-serif", body: "'Inter', sans-serif" };
  ...
  return (
    <div className="bg-teal-50 text-teal-900 py-17 px-5" style={{ fontFamily: fonts.body, backgroundColor: colors.background, color: colors.text }}>
        ...
        {!isOpen && (
          ...
          >
            ...
            <div className="relative z-10 max-w-xl mx-auto flex flex-col items-center justify-center h-full pt-10">
              ...
              <motion.button
                onClick={handleOpen}
                className="hover:scale-105 hover:shadow-teal-500/50 px-8 py-4 rounded-full font-medium tracking-widest text-xs uppercase shadow-xl transition-all duration-500 flex items-center gap-3 border"
